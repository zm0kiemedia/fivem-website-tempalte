...
@section('title', 'Mitglied hinzufügen')

@section('content')
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black text-white mb-2">Mitglied hinzufügen</h1>
            <p class="text-neutral-500">Füge eine neue Person zum Server-Team hinzu.</p>
        </div>
    ...
    <div class="bg-[#0a0a0a] rounded-3xl border border-white/5 p-8">
        <form action="{{ route('admin.team.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Rank Order -->
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-neutral-500 mb-2">Rang Ordnung (Sortierung)</label>
                    <input type="number" name="rank_order" value="99" required
                           class="w-full bg-black/20 border border-white/5 rounded-xl py-3 px-4 text-white placeholder-neutral-600 focus:outline-none focus:border-[var(--theme-primary)] focus:ring-1 focus:ring-[var(--theme-primary)] transition-all"
                           placeholder="1 = Höchster Rang">
                </div>
            ...
            <!-- Image -->
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-neutral-500 mb-2">Profilbild (Avatar)</label>
                <div class="relative group cursor-pointer">
                    ...
                    <div class="border-2 border-dashed border-white/10 rounded-xl p-8 text-center transition-all group-hover:border-[var(--theme-primary)]/50 group-hover:bg-[var(--theme-primary)]/5">
                        ...
                        <p class="text-neutral-500 text-sm font-bold">Bild auswählen</p>
                        <p class="text-neutral-600 text-xs mt-1">Quadratisch empfohlen</p>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="bio" rows="4"
                          class="w-full bg-black/20 border border-white/5 rounded-xl py-3 px-4 text-white placeholder-neutral-600 focus:outline-none focus:border-[var(--theme-primary)] focus:ring-1 focus:ring-[var(--theme-primary)] transition-all"
                          placeholder="Eine kurze Vorstellung..."></textarea>
            </div>
