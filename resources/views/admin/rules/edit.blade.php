...
@section('title', 'Regel bearbeiten')

@section('content')
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black text-white mb-2">Regel bearbeiten</h1>
            <p class="text-neutral-500">Bearbeite eine existierende Regel.</p>
        </div>
    ...
    <div class="bg-[#0a0a0a] rounded-3xl border border-white/5 p-8" x-data="{ section: '{{ $rule->section }}' }">
        <form action="{{ route('admin.rules.update', $rule) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ $rule->sort_order }}" required
            ...
            <div>
                ...
                <textarea name="content" rows="4" required
                          class="w-full bg-black/20 border border-white/5 rounded-xl py-3 px-4 text-white placeholder-neutral-600 focus:outline-none focus:border-[var(--theme-primary)] focus:ring-1 focus:ring-[var(--theme-primary)] transition-all"
                          placeholder="Der Text der Regel...">{{ $rule->content }}</textarea>
            </div>
            ...
            <div x-show="section === 'zones'" x-transition 
                ...
                <div>
                    ...
                    <select name="color"
                            class="w-full bg-black/20 border border-white/5 rounded-xl py-3 px-4 text-white placeholder-neutral-600 focus:outline-none focus:border-[var(--theme-primary)] focus:ring-1 focus:ring-[var(--theme-primary)] transition-all appearance-none cursor-pointer">
                        <option value="" {{ $rule->color == '' ? 'selected' : '' }}>Standard</option>
                        <option value="red" {{ $rule->color == 'red' ? 'selected' : '' }}>Rot (Redzone)</option>
                        <option value="green" {{ $rule->color == 'green' ? 'selected' : '' }}>Grün (Safezone)</option>
                        <option value="blue" {{ $rule->color == 'blue' ? 'selected' : '' }}>Blau</option>
                    </select>
                </div>
            </div>
